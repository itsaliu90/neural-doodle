...
def region_style_loss(style_image, target_image, style_mask, target_mask):
    ...
    s = gram_matrix(masked_style) / K.mean(K.cast(style_mask, dtype='float32')) / K.cast(num_channels, dtype='float32')
    c = gram_matrix(masked_target) / K.mean(K.cast(target_mask, dtype='float32')) / K.cast(num_channels, dtype='float32')
